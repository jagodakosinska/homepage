...
fetch(url)
  .then((resp) => resp.json())
  .then((resp) => {
    projectList = resp.filter((repo) => repo.description !== "Portfolio");
  ...
  <a class="project__link project__link--github" href="${html_url}" target="_blank" rel="nofollow noreferrer" title="Source code: ${name}">Github</a>    </div></li>`;
    });
  })
  .catch((err) => {
    console.log(err);
